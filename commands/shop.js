...
module.exports = {
    ...
    execute: async (message, args, bot, prefix) => {
        ...
        if (["survivor", "surv", "survivors"].includes(args[1].toLowerCase())) {
            ...
            else if (Number(args[2]) > skinPages.length) page = skinPages.length - 1;
            else if (args[2] == 0) page = 0;
            else if (Math.sign(args[2]) == -1 && args[2] < -5) page = 0;
            else if (Math.sign(args[2]) == -1) page = Math.abs(Number(args[2]) + 1)
            else page = Math.abs(Number(args[2]) - 1);
